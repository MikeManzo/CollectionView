...
public final class CollectionViewGridLayout: CollectionViewLayout {
    ...
    public final var columnCount: NSInteger = 2 { didSet { invalidate() }}
    
    /// The spacing between each column
    public final var columnSpacing: CGFloat = 8 { didSet { invalidate() }}
    ...
    public final var aspectRatio: CGSize = CGSize(width: 1, height: 1) { didSet { invalidate() }}
    ...
    public final var sectionInsets: NSEdgeInsets = NSEdgeInsets(top: 8, left: 8, bottom: 8, right: 8) { didSet { invalidate() }}
    ...
    fileprivate var numSections: Int { return self.collectionView!.numberOfSections }
    ...
    fileprivate weak var delegate: CollectionViewDelegateGridLayout? {
    ...
    override public func prepare() {
        ...
        for section in 0..<numberOfSections {
            ...
            let sectionInsets =  self.delegate?.collectionView?(self.collectionView!, layout: self, insetForSectionAt: section)
                ?? self.sectionInsets
            let rowSpacing = self.delegate?.collectionView?(self.collectionView!, layout: self, rowSpacingForSectionAt: section)
                ?? self.interitemSpacing
            let colSpacing = self.delegate?.collectionview?(self.collectionView!, layout: self, columnSpacingForSectionAt: section)
                ?? self.columnSpacing
            ...
            if let aSize = self.delegate?.collectionView?(self.collectionView!, layout: self,
                                                          aspectRatioForItemsInSection: section), aSize.width > 0 && aSize.height > 0 {
             ...
             * 2. Section header
             */
            let heightHeader: CGFloat = self.delegate?.collectionView?(self.collectionView!, layout: self,
                                                                       heightForHeaderInSection: section) ?? self.headerHeight
            if heightHeader > 0 {
                let attributes = CollectionViewLayoutAttributes(forSupplementaryViewOfKind: CollectionViewLayoutElementKind.SectionHeader,
                                                                with: IndexPath.for(item: 0, section: section))
                attributes.alpha = 1
                attributes.frame = insetSupplementaryViews ?
                    CGRect(x: sectionInsets.left, y: top, width: self.collectionView!.bounds.size.width - sectionInsets.width, height: heightHeader)
            ...
            let footerHeight = self.delegate?.collectionView?(self.collectionView!, layout: self,
                                                              heightForFooterInSection: section) ?? self.footerHeight
            if footerHeight > 0 {
                let attributes = CollectionViewLayoutAttributes(forSupplementaryViewOfKind: CollectionViewLayoutElementKind.SectionFooter,
                                                                with: IndexPath.for(item: 0, section: section))
                attributes.alpha = 1
                attributes.frame = insetSupplementaryViews ?
                    CGRect(x: sectionInsets.left, y: top, width: self.collectionView!.bounds.size.width - sectionInsets.width, height: footerHeight)
    ...
    public func indexPathsForItemsInRect(_ rect: CGRect) -> Set<IndexPath>? {
        //        return nil
    ...
    public func layoutAttributesForElementsInRect(_ rect: CGRect) -> [CollectionViewLayoutAttributes]? {
    ...
    public func layoutAttributesForItemAtIndexPath(_ indexPath: IndexPath) -> CollectionViewLayoutAttributes? {
        return itemAttributes[indexPath]
    }
    
    public func layoutAttributesForSupplementaryViewOfKind(_ elementKind: String, atIndexPath indexPath: IndexPath) -> CollectionViewLayoutAttributes? {
    ...
    public func shouldInvalidateLayoutForBoundsChange (_ newBounds: CGRect) -> Bool {
    ...
    public func scrollRectForItemAtIndexPath(_ indexPath: IndexPath, atPosition: CollectionViewScrollPosition) -> CGRect? {
    ...
    public func indexPathForNextItemInDirection(_ direction: CollectionViewDirection, afterItemAtIndexPath currentIndexPath: IndexPath) -> IndexPath? {
        ...
        guard let cellRect = collectionView.rectForItem(at: currentIndexPath) else { return nil }
